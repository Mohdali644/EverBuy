      ...
      >
        ...
        <div className="px-5 py-8 border-b border-slate-200 flex justify-between items-center bg-slate-900 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-[1.34rem] font-black text-white m-0">
              Your Manifest
            </h2>
          </div>
          <button
        ...
        <div className="px-5 py-4 bg-slate-50 border-b border-slate-200 shrink-0">
          {cartTotal >= THRESHOLD ? (
            <p className="text-emerald-600 font-bold text-[0.85rem] flex items-center gap-2 m-0 mb-2">
              <i className="fa-solid fa-check-circle"></i> Prime Shipping
              Unlocked!
            </p>
          ) : (
        ...
        <div className="flex-grow overflow-y-auto p-5 flex flex-col gap-3 scrollbar-hide bg-white">
          {cart.length === 0 ? (
            ...
            cart.map((item, index) => (
              <div
                key={index}
                className="group flex gap-4 bg-white p-3 rounded-2xl border border-slate-200 hover:border-slate-300 transition-all shadow-sm hover:shadow-md"
              >
                {/* Item Image */}
                ...
                <div className="flex-grow flex flex-col justify-between py-0.5">
                  <div className="flex justify-between items-start gap-2">
                    <h3 className="text-slate-900 font-bold text-[0.9rem] leading-snug line-clamp-2 m-0">
                      {item.title}
                    </h3>
                    <button
                  ...
                  <div className="flex justify-between items-end mt-1">
                    <span className="text-[#ff9900] font-black text-[1rem]">
