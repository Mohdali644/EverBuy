    ...
    <>
      ...
      >
        ...
        >
          ...
          <nav
            className={`flex flex-wrap items-center justify-between px-3 sm:px-5 gap-x-2 gap-y-2 sm:gap-y-3 transition-all duration-500 ${isScrolled ? "py-2 md:h-auto" : "py-3 md:h-auto max-w-[1800px] mx-auto"}`}
          >
            {/* Logo */}
            <div className="flex-shrink-0">
              <Link
                to="/"
                className="flex items-center gap-1.5 sm:gap-2 no-underline transition-transform duration-300 hover:scale-105 group/logo"
              >
                ...
                <div className="flex items-baseline font-sans tracking-tight">
                  <span className="text-[1.4rem] sm:text-[1.6rem] font-black text-white drop-shadow-sm">
                    Ever
                  </span>
                  <span className="text-[1.4rem] sm:text-[1.6rem] font-black bg-clip-text text-transparent bg-gradient-to-br from-[#FF9900] to-[#FF3300] italic drop-shadow-sm">
                    Buy
                  </span>
                </div>
              </Link>
                ...
                <div className="absolute top-[50px] md:top-[55px] left-0 w-full bg-[#131921]/95 backdrop-blur-3xl rounded-2xl shadow-[0_30px_60px_rgba(0,0,0,0.6)] z-[9999] overflow-hidden border border-white/10 ring-1 ring-white/5 p-2">
                  {results.length > 0 ? (
                    results.map((item, idx) => (
                      ...
                      >
                        <i className="fa-solid fa-arrow-turn-up text-[#ff9900] rotate-90 text-[0.7rem] mr-3"></i>{" "}
                        {item}
                      </div>
                    ))
                  ) : (
            ...
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              >
                ...
                <div className="hidden md:flex flex-col">
                  <span className="text-[0.75rem] text-slate-400 font-medium leading-none group-hover:text-white transition-colors">
                    Returns
                  </span>
                  <span className="text-[0.95rem] text-white font-bold leading-tight tracking-wide">
                    & Orders
                  </span>
                </div>
              ...
              >
                <div className="relative flex items-center justify-center w-8 h-8 sm:w-9 sm:h-9 rounded-full border border-transparent group-hover/cart:border-[#ff9900]/50 transition-all duration-300">
                  <span className="absolute -top-1 -right-1 text-white bg-[#ff3300] px-1.5 py-[1px] rounded-full font-bold text-[0.65rem] shadow-sm border border-[#131921] z-10">
                    {cart.length}
                  </span>
                ...
                <div className="hidden lg:flex flex-col justify-center">
                  <span className="text-[0.75rem] text-slate-400 font-medium group-hover/cart:text-white tracking-wide transition-colors">
                    Manifest
                  </span>
                  <span className="text-[0.95rem] font-bold text-[#ff9900] tracking-wide leading-none">
          ...
          <div
            className={`transition-all duration-500 sm:overflow-x-hidden overflow-x-scroll overflow-y-hidden scrollbar-hide flex-nowrap whitespace-nowrap flex items-center px-4 sm:px-6 gap-6 text-[0.8rem] sm:text-[0.85rem] font-medium max-w-[1800px] mx-auto tracking-wide overscroll-x-contain touch-pan-x ${isScrolled ? "h-0 py-0 opacity-0 border-transparent pointer-events-none" : "h-[35px] sm:h-[40px] py-[21px] opacity-100 border-t border-white/10"}`}
          >
            <button
              className="group flex items-center gap-1.5 px-3 py-1 rounded-full shrink-0 text-white bg-white/10 hover:bg-gray-100 hover:text-black transition-all outline-none focus:outline-none"
              onClick={() => setInfoOpen("All Categories")}
            >
              <i className="fa-solid fa-bars text-base transition-transform duration-300 group-hover:rotate-90"></i>
              <span>All</span>
            </button>

            <button
              onClick={() => setComingSoon("Today's Deals")}
              className="px-3 py-1 ml-6 mr-6 rounded-full text-slate-100 bg:text-white bg-white/10 hover:bg-gray-100 hover:text-black transition-all outline-none focus:outline-none shrink-0"
            >
              Today's Deals
            </button>
            <button
              onClick={() => setComingSoon("Customer Service")}
              className="px-3 py-1 mr-7 rounded-full text-slate-100 bg:text-white bg-white/10 hover:bg-gray-100 hover:text-black transition-all outline-none focus:outline-none shrink-0"
            >
              Customer Service
            </button>
            <button
              onClick={() => setComingSoon("Registry")}
              className="px-3 py-1 mr-7 rounded-full text-slate-100 bg:text-white bg-white/10 hover:bg-gray-100 hover:text-black transition-all outline-none focus:outline-none shrink-0"
            >
              Registry
            </button>
            <button
              onClick={() => setComingSoon("Gift Cards")}
              className="px-3 py-1 mr-7 rounded-full text-slate-100 bg:text-white bg-white/10 hover:bg-gray-100 hover:text-black transition-all outline-none focus:outline-none shrink-0"
            >
              Gift Cards
            </button>
          ...
          <div className="relative w-full max-w-[320px] bg-[#0a0f16] rounded-[24px] p-6 shadow-[0_20px_40px_rgba(0,0,0,0.5),0_0_30px_rgba(255,153,0,0.05)] border border-white/10 animate-in zoom-in-95 fade-in duration-200 overflow-hidden">
            ...
            <div className="flex flex-col items-center text-center relative z-10 mt-1">
              ...
              <p className="text-slate-400 font-medium text-[0.85rem] leading-snug mb-5 px-2">
                <span className="text-[#ff9900] tracking-wide font-bold">
                  {comingSoon}
                </span>{" "}
                is currently undergoing polishing.
              </p>
